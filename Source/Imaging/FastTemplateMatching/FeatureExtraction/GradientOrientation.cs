...
namespace LINE2D
{
    public unsafe static class GradientOrientation
    {
        public const int INVALID_ORIENTATION = 360 + 1; //used where magnitude to small; 360+1 => look quantization table

        public static void ComputeGradient(Image<Bgr, Byte> frame, out Image<Gray, int> orientImg, int minValidMagnitude)
        {
            ...
            for (int j = 0; j < imgHeight; j++)
            {
                for (int i = 0; i < imgWidth; i++)
                {
                    ...
                    if (mag < minValidMagSqr)
                        *orientImgPtr = INVALID_ORIENTATION;
                    else
                    {
        ...
        public static void ComputeGradient(Image<Gray, Byte> frame, out Image<Gray, int> orientImg, int minValidMagnitude)
        {
            int minValidMagSqr = minValidMagnitude * minValidMagnitude;
