...
namespace GOCO
{
    ...
    public class GentleBoost<TWeakLearner>
    {
        ...
        public float Evaluate(Func<TWeakLearner, float> learnerClassifyFunc) 
        {
            float output = 0;
        ...
        /// </summary>
        /// <param name="isPositive">Requested outputs (e.g. for classification the valid values are +1 and -1).</param>
        /// <param name="outputs">
        ...
        /// </param>
        public void Train(IList<bool> isPositive, IList<float> outputs,
                          Func<float[], TWeakLearner> learnerCreateAndTrainFunc, 
                          Func<TWeakLearner, int, float> learnerClassifyFunc,
                          Func<IList<TWeakLearner>, IList<float>, bool> terminationFunc) 
        {
            int nSamples   = isPositive.Count;
            int nPositves  = isPositive.Where(x=> x == true).Count();
            ...
            do
            {
                ...
                //for (int i = 0; i < nSamples; i++)
                {
                    ...
                    if (nProcessedSamples % 100 == 0 || nProcessedSamples == nSamples)
