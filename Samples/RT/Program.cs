...
namespace RT
{
    static class Program
    {
        ...
        static void Train()
        {
            ...
            var picoClassifier = new PicoClassifier(1);
        ...
        {
            string dir = @"S:\Svjetla - baza podataka - Boris\Prednja svjetla\";

            Database db = new Database();
            //db.Load(Path.Combine(dir, "1.xml"));
            db.Load(Path.Combine(dir, "2.xml"));
            ...
            db.Export((imgKey) => 
            {
                var filePath = Path.Combine(dir.NormalizePathDelimiters(), imgKey.NormalizePathDelimiters().TrimStart(Path.DirectorySeparatorChar));
        ...
        static void trainCarSamples()
        {
            ...
            var picoClassifier = new PicoClassifier(2.2f);
