...
namespace ParticleFilterModelFitting
{
    public class ParticleState: ICloneable
    {
        ...
        public static void Difuse(ref ParticleState state)
        {
            ...
            {
                X = state.Position.X + 2.5f * (float)normalDistribution.Generate(),
                Y = state.Position.Y + 2.5f * (float)normalDistribution.Generate(),
            };

            state.Scale = Math.Max(200, state.Scale + 1.5f * (float)normalDistribution.Generate());
        ...
        public override bool Equals(object obj)
        {
            ...
            if (state.Position.Equals(this.Position) && state.Scale == this.Scale && state.RotationZ == this.RotationZ)
            {
                return true;
            }
